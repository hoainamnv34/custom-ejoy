function NRSelectionDetector() {
  ...
  function initSelectEventListener() {
    window.addEventListener('mouseup', e => {
      try {
        ejoyChangeSpeedPopup && ejoyChangeSpeedPopup.setShowHide(false);
      } catch (err) {
  ...
  function getSelected() {
    ...
    if (window.getSelection) {
      if (document.URL.toString().includes('https://www.youtube.com')) t = window.getSelection().isCollapsed ? '' : window.getSelection();
      else t = window.getSelection();
    } else if (document.getSelection) {
      if (document.URL.toString().includes('https://www.youtube.com')) t = document.getSelection().isCollapsed ? '' : document.getSelection();
      else t = document.getSelection();
  ...
  function showReadingIcon(x, y) {
    const range = getSelected().getRangeAt(0);
    const box = range.getBoundingClientRect();
    let leftIcon = x;
    let topIcon = y;
    const scrollTop = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop; // distance from top of brower screen to curr top of doc
    if (x !== undefined) {
      const midleLine = scrollTop + box.top + box.height / 2;
      leftIcon = `${ x + sizeIcon }px`;
      topIcon = `${ (Math.round(y > midleLine ? window.pageYOffset + box.bottom - sizeIcon * 2 : window.pageYOffset + box.top + sizeIcon)) - 8 }px`;
    }
    self.frame.style.top = topIcon;
    self.frame.style.left = leftIcon;
