var doc = doc && doc.type === 'htmlDoc' ? doc : new function() {
        ...
        return new Promise(async (resolve) => {
            ...
            .then(() => {
                return new Promise((resolve) => {
                    let start = new Date();
                    let textBlocks = findTextBlocks(1000);
        ...
        return new Promise(async (resolve) => {
            ...
            if ($(elem).data("read-aloud-multi-block")) {
                ...
                for (let i = 0; i < children.length; i++) {
                    if(children[i].tagName !== 'NR-SENTENCE' && !children[i].classList.contains('nr-selected')){
                        await traverseDOM(children[i], lastSelectedNode);
                    }
                }
            } else {
                if(elem.tagName !== 'NR-SENTENCE' && !elem.classList.contains('nr-selected')){
    ...
    async function traverseDOM(node, lastSelectedNode = null, op) {
        ...
        try {
            ...
            if (children.length === 0) {
                ...
                if (node.nodeName.toLowerCase() === 'p' || ($(node).children('ul').length === 0 && node.nodeName.toLowerCase() === 'li')) {
    ...
    async function setSentencesHelper(textNode, sentences, originalSentences, index) {
        try {
            ...
            let ttsSentence = sentences[index].trim();
            let nextIndex = index;
