...
import '../../chrome/extension/inject';
...
const nav = {
  ...
  openSetting() {
    ...
    window.open(`chrome-extension://${chrome.runtime.id}/options/index.html?screen=readerView`)
  }
};
window.nav = nav;
...
const update = {
  async: () => {
    ...
    //
    document.querySelector('[data-id="font-size"]').textContent = prefs['font-size'] + 'px';
    document.querySelector('[data-id="screen-width"]').textContent = prefs['width'] || 'unset';
    document.querySelector('[data-id="line-height"]').textContent = lh;
  },
  images: () => {
...
imageUtils.querySelector('tr').title = locales('reader_btn_plus_minus_controls')
...
/* printing */
{
  ...
  span.title = `${locales('reader_print_title_desc')} (command)'`;
...
{
  const span = document.createElement('span');
  span.title = `${locales('reader_email_title_desc')} (command)`;
...
{
  const span = document.createElement('span');
  span.title = `${locales('reader_save_title_desc')} (command)`;
...
{
  const span = document.createElement('span');
  span.title = `${locales('reader_fullscreen_title_desc')} (command)`;
...
/* design mode */
{
  ...
  span.title = `${locales('reader_toggle_title_desc')} (command)

${locales('reader_toggle_title_desc_1')}

Ctrl/Command + B: ${locales('reader_toggle_title_desc_2')}
Ctrl/Command + I: ${locales('reader_toggle_title_desc_3')}
Ctrl/Command + U: ${locales('reader_toggle_title_desc_4')}`;
...
{
  const span = document.createElement('span');
  span.title = `${locales('reader_article_title_desc')} (command)\n${locales('reader_article_title_desc_1')}`;
  ...
  span.onclick = async () => {
    ...
    if (document.body.dataset.speech === 'true') {
      ...
      let content = iframe.contentDocument.querySelectorAll('.page p, .page h1, .page h2, .page h3, .page h4, .page li, .page td, .page th');
      if (content.length <= 0) content = iframe.contentDocument.querySelectorAll('.page article');
      tts.feed(...content);
...
/* images */
{
  ...
  span.title = `${locales('reader_image_title_desc')} (command)`;
...
/* highlight */
{
  ...
  span.title = `${locales('reader_highlight_title_desc')} (command)`;
  ...
  config.load(() => {
    try {
      for (const action of config.prefs['user-action']) {
        ...
        span.title = action.title || locales('reader_User_Action');
...
console.log('dfdffdfđffđf', document.querySelectorAll('[data-cmd]'))
const dataCmds = (document.querySelectorAll('[data-cmd]') || []);

[].forEach.call(dataCmds, function(dataCmd) {
  const valueDataCmd = dataCmd.getAttribute('data-cmd');
  switch (valueDataCmd) {
    case 'close':
      dataCmd.title = locales('reader_close')
      break;
    case 'setting':
      dataCmd.title = locales('reader_open_settings')
      break;
    case 'open-font-utils':
      dataCmd.title = locales('reader_btn_plus_minus_controls')
      break;
    default:
      break;
  }
...
document.getElementById('navigate-previous').title = `${locales('reader_btn_prev_controls')} (command)`
document.getElementById('navigate-next').title = `${locales('reader_btn_next_controls')} (command)`
...
// load
config.load(() => {
  console.log('config.prefs', config.prefs['mail-button'])
  document.body.dataset.mode = config.prefs.mode;
